...
import { VehicleTrackingRedis } from './tracking-redis';
import { createLogger } from '../../utils/logger';

const logger = createLogger('vehicle-tracker');
...
export class VehicleTracker {
  ...
  constructor(trailConfig?: Partial<TrailConfig>, deviceId?: string) {
    ...
    logger.info('VehicleTracker initialized with Redis storage', { deviceId: this.deviceId });
  }
  ...
  public async processObjectData(objectData: ObjectData): Promise<TrackingUpdate> {
    ...
    if (!objectData.entries || !Array.isArray(objectData.entries)) {
      logger.error('ObjectData missing entries', { objectData });
      return {
  ...
  private isVehicleInDetectionZone(position: VehiclePosition): boolean {
    return (
      position.x >= DETECTION_ZONE.minX &&
  ...
  public updateTrailConfig(newConfig: Partial<TrailConfig>): void {
    ...
    logger.info('VehicleTracker trail config updated', { config: this.trailConfig });
  }

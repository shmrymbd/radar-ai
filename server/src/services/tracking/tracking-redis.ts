...
const logger = createLogger('tracking-redis');
...
export class VehicleTrackingRedis {
  ...
  public async setVehicleState(targetId: string, vehicle: VehicleState): Promise<void> {
    try {
      ...
      logger.error('Error storing vehicle state', { targetId, error });
      throw error;
    }
  }
  ...
  public async batchSetVehicleStates(vehicles: Map<string, VehicleState>): Promise<void> {
    ...
    try {
      ...
      logger.error('Error batch storing vehicle states', { error });
      throw error;
    }
  }
  ...
  public async getVehicleState(targetId: string): Promise<VehicleState | undefined> {
    try {
      ...
      logger.error('Error getting vehicle state', { targetId, error });
      return undefined;
    }
  }
    ...
    try {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        try {
          ...
          logger.error('Error parsing vehicle state', { targetId, error: parseError });
          orphanedIds.push(targetId);
        }
      }
      ...
      if (orphanedIds.length > 0) {
        this.removeOrphanedIds(orphanedIds).catch((err) =>
          logger.error('Error removing orphaned IDs', { error: err })
        );
      }

      return vehicles;
    } catch (error) {
      logger.error('Error getting all vehicle states', { error });
      return [];
    }
  }
  ...
  private async removeOrphanedIds(targetIds: string[]): Promise<void> {
    try {
      ...
      await pipeline.exec();
      logger.info('Removed orphaned vehicle IDs from Set', { count: targetIds.length });
    } catch (error) {
      logger.error('Error removing orphaned IDs', { error });
      throw error;
    }
  }
  ...
  public async batchGetVehicleStates(targetIds: string[]): Promise<Map<string, VehicleState>> {
    ...
    try {
      ...
      if (missCount > 0) {
        ...
        logger.debug('Batch get stats', { hitCount, missCount, hitRate });
      }

      return vehicles;
    } catch (error) {
      logger.error('Error batch getting vehicle states', { error });
      return new Map();
    }
  }
   ...
   */
  public async addToVehicleHistory(
    targetId: string,
    ...
    try {
      ...
      logger.error('Error adding to vehicle history', { targetId, error });
    }
  }
   ...
   */
  public async batchAddToVehicleHistories(
    updates: Map<string, VehiclePosition>,
    maxLength: number = 50
  ): Promise<void> {
    ...
    try {
      ...
      logger.error('Error batch adding to vehicle histories', { error });
    }
  }
  ...
  public async batchGetVehicleHistories(targetIds: string[]): Promise<Map<string, VehiclePosition[]>> {
    ...
    try {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        if (history && history.length > 0) {
          try {
            histories.set(
              targetId,
              history.map((item: string) => JSON.parse(item))
            );
          } catch (parseError) {
            logger.error('Error parsing vehicle history', { targetId, error: parseError });
      ...
      logger.error('Error batch getting vehicle histories', { error });
      return new Map();
    }
  }
  ...
  public async getVehicleHistory(targetId: string): Promise<VehiclePosition[]> {
    try {
      ...
      logger.error('Error getting vehicle history', { targetId, error });
      return [];
    }
  }
  ...
  public async deleteVehicle(targetId: string): Promise<void> {
    try {
      ...
      if (removed > 0) {
        logger.debug('Deleted vehicle from tracking', { targetId });
      }
    } catch (error) {
      logger.error('Error deleting vehicle', { targetId, error });
      throw error;
    }
  }
    ...
    try {
      ...
      for (const targetId of vehicleIds) {
        ...
        if (!exists) {
          ...
          orphanedCount++;
          logger.debug('Removed orphaned vehicle ID', { targetId });
          continue;
        }
      ...
      if (removedCount > 0 || orphanedCount > 0) {
        ...
        logger.info('Cleanup complete', {
          expiredVehicles: removedCount,
          orphanedIds: orphanedCount,
  ...
  public async clearAll(): Promise<void> {
    try {
      ...
      logger.error('Error clearing all tracking data', { error });
    }
  }
}

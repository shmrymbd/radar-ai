...
const logger = createLogger('passdata-mongodb');
...
export class PassDataMongoDBService {
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      logger.info('MongoDB PassData indexes created');
    } catch (error) {
  ...
  public async getClassificationMetrics(deviceId: string) {
    try {
      ...
      const vehicleTypes = await collection
        .aggregate([
          { $match: { deviceId } },
          {
            $group: {
      ...
      const laneUtilization = await collection
        .aggregate([
          { $match: { deviceId } },
          {
            $group: {
      ...
      const averageSpeeds = await collection
        .aggregate([
          { $match: { deviceId } },
          {
            $group: {
      ...
      const formattedAverageSpeeds = averageSpeeds.map((item: any) => {
        const total =
          item.speedDistribution['0-20'] +
          item.speedDistribution['20-40'] +
          item.speedDistribution['40-60'] +
          item.speedDistribution['60-80'] +
      ...
      logger.error('Error getting classification metrics from MongoDB', { error });
      throw error;
    }
  }
  ...
  public async getClassificationSummary(deviceId: string) {
    try {
      ...
      const avgSpeedResult = await collection
        .aggregate([
          { $match: { deviceId } },
      ...
      const laneUtilResult = await collection
        .aggregate([
          { $match: { deviceId } },
      ...
      const laneUtilization = laneUtilResult.length > 0 ? laneUtilResult[0].avgUtil / (totalVehicles || 1) : 0;
      ...
      const trafficComposition = await collection
        .aggregate([
          { $match: { deviceId } },
          {
            $group: {
      ...
      logger.error('Error getting classification summary from MongoDB', { error });
      throw error;
    }
  }
}
